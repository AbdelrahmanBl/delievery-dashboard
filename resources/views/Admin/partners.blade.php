                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <div class="avtar-pic w35 bg-red" data-toggle="tooltip" data-placement="top" title="{{$Order->markerter_name}}"><span>{{substr($Order->markerter_name, 0, 1)}}</span></div>
                                                        <div class="ml-3">
                                                                <p class="mb-0">{{$Order->markerter_name}}</p>
                                                            <p class="mb-0">{{$Order->markerter_code}}</p>
                                                        </div>
                                                    </div>                                        
                                                </td>
                                                <td>966 {{$Order->markerter_mobile}}</td>
                                                <td>SAR {{$Order->markerter_wallet_balance}}</td>
                                                <td>{{$Order->markerter_orders}}</td>
                                                <td>{{$Order->customer_discount}}%</td>
                                                <td>{{$Order->markerter_commission}} + {{$Order->markerter_commission_per}} %</td>
                                                <td><span class="badge badge-success ml-0 mr-0">{{$Order->markerter_status}}</span></td>
                                                <td>
                                                    <button type="button" onclick="loadWallet({{$Order->markerter_id}})" id="walletBtn{{$Order->markerter_id}}" class="btn btn-sm btn-default " title="Print" data-toggle="tooltip" data-placement="top"><i class="icon-wallet"></i></button>
                                                    <button onclick="updateStatus({{$Order->markerter_id}})" id="statusBtn{{$Order->markerter_id}}" type="button" class="btn btn-sm btn-default" title="Delete" data-toggle="tooltip" data-placement="top"><i class="icon-trash text-danger"></i></button>
                                                </td>
                                            </tr>
                                            @endforeach
                                            @endif
