                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover table-custom spacing8">
                                        ...
                                        <tbody>
                                            ...
                                            <tr id="DIVOFDOC{{$Promocode->id}}">
                                                    <td>{{$Promocode->promo_code}}</td>
                                                <td>
                                                        <div class="d-flex align-items-center">
                                                                <div class="avtar-pic w35 bg-red" data-toggle="tooltip" data-placement="top" title="{{$Promocode->name}}"><span><img width="100%" src="{{$Promocode->image}}"></span></div>
                                                                <div class="ml-3">
                                                                        <p class="mb-0">{{$Promocode->name}}</p>
                                                                    <p class="mb-0">{{$Promocode->mobile}}</p>
                                                                </div>
                                                            </div> 
                                                </td>
                                                <td><span id="percentage{{$Promocode->id}}">{{$Promocode->percentage}}</span> %</td>
                                                <td id="max_amount{{$Promocode->id}}">{{$Promocode->max_amount}}</td>
                                                <td id="expiration{{$Promocode->id}}">{{explode(' ',$Promocode->expiration)[0]}}</td>
                                                <td><span id="status{{$Promocode->id}}" class="badge @if($Promocode->status == 'APPROVED') badge-success @else badge-danger @endif ml-0 mr-0">{{$Promocode->status}}</span></td>
                                                <td>{{$Promocode->used}}</td>
                                                <td>
                                                        <button type="button" id="updateBtn{{$Promocode->id}}" onclick="openUpdateService('{{$Promocode->id}}')" class="btn btn-sm btn-default" title="Edit"><i class="fa fa-edit"></i></button>
                                                        <button onclick="deletePromocode('{{$Promocode->id}}')" type="button" class="btn btn-sm btn-default js-sweetalert" title="Delete" ><i class="fa fa-trash-o text-danger"></i></button>
                                                    </td>
                                            </tr>
                                            @endforeach
                                            @endif
